...
- 与 Flash 一致的录音中状态：录音开始时出现，录音结束时退出。
...
Demo 网页提供一个弱化但可发现的 Operator Controls 入口，供展会工作人员使用，不把维护操作放进访客的主体验路径。首版至少包含“重置 Demo 数据”。

重置需要二次确认。成功后当前页面清除本地 transcript、结果和历史状态，回到可开始下一轮演示的初始状态；戒指保持物理连接。
...
展会反复体验使用一个普通 UReka Demo 账号。该账号可以同时登录 `ring-demo` 和现有 UReka 客户端，看到同一份 Backend/MySQL 数据。

Backend 新增受 JWT 保护的 `POST /api/demo/reset`：

- 只清理当前登录用户的数据，绝不执行全局清库。
- 默认关闭；只有根目录 `.env` 中 `DEMO_RESET_ENABLED=true` 时才注册或启用。
- 清理该用户的 Assets、Asset Fields、Contacts、Events、Flash Sessions、Input Turns、Messages，以及由这些演示产生的相关通知、文件、任务和报告等工作区内容。
- 保留 User、密码、User Skills/Render Specs、Connected Apps、系统级 Global Skills 和戒指桌面配置。
- 在一个数据库事务中按外键依赖顺序执行；任何一步失败则整体回滚。
- 返回按数据类型统计的删除数量，供 Operator Controls 显示确认结果。

Demo Web 的 Reset 入口放在 Operator Controls 中：

1. 工作人员点击 Reset。
2. 页面明确说明只会清空当前 Demo 账号，并要求二次确认。
3. 页面调用 `/api/demo/reset`。
4. 成功后清除当前 Demo 的前端状态和缓存，回到 Ready。
5. 失败时保留当前内容并显示维护提示，不触发戒指失败震动。

当前仓库的全局 `backend/scripts/clear_data.py` 不用于网页 Reset，也不暴露给 Demo。
...
- Demo Reset：覆盖开关关闭、未登录、仅删除当前用户、保留账号配置、事务回滚和删除统计。
...
11. 在 Operator Controls 中重置 Demo，确认 UReka 客户端中的该账号内容同步清空，但账号、Skills 和连接配置仍在。
...
7. 增加按用户隔离的 Demo Reset API 和网页 Operator Controls。
8. 增加模拟测试、全链路测试和真实戒指验证。
9. 补充同事本地 Setup/Run 文档与启动脚本。
