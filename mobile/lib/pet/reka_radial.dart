...
class _RekaRadialState extends State<RekaRadial> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    final panel = Positioned(
      left: left,
      width: _panelW,
      top: openAbove ? null : (a.bottom + 10).clamp(mq.padding.top + 8, h - 120),
      bottom: openAbove ? (h - a.top + 10).clamp(12.0, h - 80) : null,
      child: ScaleTransition(
        scale: Tween(begin: 0.96, end: 1.0).animate(CurvedAnimation(parent: _c, curve: Curves.easeOut)),
        ...
        children: [
          Positioned.fill(
            child: GestureDetector(
              ...
              onTap: _dismiss,
              child: FadeTransition(opacity: _c, child: ColoredBox(color: Colors.black.withValues(alpha: 0.08))),
            ),
          ),
          panel,
        ],
  ...
  Widget _glassPanel(EurekaColors eu) {
    ...
    final topC = Color.alphaBlend(t0.withValues(alpha: 0.30), const Color(0xBC10161E));
    final botC = Color.alphaBlend(t1.withValues(alpha: 0.16), const Color(0xC70C111E));
      ...
      child: BackdropFilter(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            gradient: LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [topC, botC]),
            borderRadius: BorderRadius.circular(22),
            border: Border.all(color: border),
            boxShadow: [
              BoxShadow(color: Colors.black.withValues(alpha: 0.55), blurRadius: 44, offset: const Offset(0, 18)),
              BoxShadow(color: t0.withValues(alpha: 0.42), blurRadius: 30, spreadRadius: -6),
            ],
          ),
          child: Wrap(
  ...
  Widget _item(EurekaColors eu, int i) {
    ...
    final anim = CurvedAnimation(parent: _c, curve: Interval(begin, (begin + 0.4).clamp(0.0, 1.0), curve: Curves.easeOutBack));
    return AnimatedBuilder(
      animation: anim,
      builder: (context, child) {
        final v = anim.value.clamp(0.0, 1.0);
        return Opacity(opacity: v, child: Transform.scale(scale: 0.55 + 0.45 * v, child: child));
      },
      child: GestureDetector(
        onTap: () {
          widget.onClose();
                ...
                children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(color: Colors.white.withValues(alpha: it.accent ? 0.0 : 0.16)),
                    ),
                    ...
                    Positioned(
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          border: Border.all(color: const Color(0xFF0C1322), width: 2),
                        ),
                        child: Text(widget.notifCount > 99 ? '99+' : '${widget.notifCount}',
