...
class _LibraryPageState extends State<LibraryPage> {
  ...
  Future<void> _exportMenu() async {
    ...
    if (data.events.isNotEmpty) {
      types.add(ExportType('event', '事件', '📅', data.events.length));
    }
    if (data.contacts.isNotEmpty) {
      types.add(ExportType('contact', '名片', '👤', data.contacts.length));
    }
    if (types.isEmpty) {
    ...
    final eu = context.eu;
    final result =
        await showModalBottomSheet<({String format, Set<String> types})>(
          context: context,
    ...
    if (result != null && result.types.isNotEmpty) {
      await _export(result.format, result.types);
    }
  }
  ...
  Future<void> _export(String format, Set<String> types) async {
    ...
    try {
      ...
      await Share.shareXFiles([
        XFile(file.path, mimeType: mime, name: name),
      ], subject: 'UReka 导出');
    } catch (e) {
  ...
  Widget _body(EurekaColors eu, _LibData data) {
    ...
    final customGrid = gridSkills
        .where((e) => !_systemicSkills.contains(e.key))
        .toList();

    final todoMeta = data.skills['todo'] ?? const SkillMeta('✅', '待办', 'blue');
    final suijiMeta =
        data.skills['notes'] ?? const SkillMeta('✍️', '随记', 'amber');

    final recent = _buildRecent(data)
      ...
      children: [
        ...
        _SectionLabel(
          ...
          trailing: GestureDetector(
            onTap: () => Navigator.of(
              context,
            ).push(MaterialPageRoute(builder: (_) => const SkillManagePage())),
            child: Row(
  ...
  List<_RecentEntry> _buildRecent(_LibData d) {
    final out = <_RecentEntry>[
      ...
      for (final e in d.events)
        _RecentEntry(_parseTs(e), {'card_type': 'event', ...e}),
      for (final c in d.contacts)
        _RecentEntry(_parseTs(c), {'card_type': 'contact', ...c}),
    ];
    return out;
  }
  ...
  Widget _skillTile(
    ...
    return UQuietSurface(
      onTap: onTap,
      padding: const EdgeInsets.all(12),
      radius: URadii.card,
      stackDepth: _stackDepth(count),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(
            children: [
              _tileIcon(eu, accent, meta.label, meta.icon, icon),
              const Spacer(),
              _tileCount(eu, count),
            ],
          ),
  ...
  Widget _tileIcon(
    ...
    final resolved = icon ?? _fallbackIcon(label, glyph);
    return Container(
      width: 30,
      height: 30,
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: eu.brightness == Brightness.light
            ? [
                BoxShadow(
                  color: const Color(0xFF181612).withValues(alpha: 0.035),
                  blurRadius: 3,
          ...
          children: [
                  ...
                  gradient: LinearGradient(
                    ...
                    colors: [
                      Colors.white.withValues(
                        alpha: eu.brightness == Brightness.light ? 0.22 : 0.05,
                      ),
                      Colors.transparent,
                    ],
                  ),
            ...
            Center(
              child: resolved == null
                  ...
                  : Icon(resolved, size: 16, color: accent),
            ),
          ],
        ),
      ),
  ...
  IconData? _fallbackIcon(String label, String glyph) {
    if (glyph != '•' && glyph.trim().isNotEmpty) return null;
    if (label.contains('灵感')) return Icons.lightbulb_outline_rounded;
    if (label.contains('记账')) return Icons.receipt_long_rounded;
    if (label.contains('训练') || label.contains('跑步')) {
      return Icons.directions_run_rounded;
    }
    if (label.contains('喝水') || label.contains('饮食')) {
      return Icons.water_drop_outlined;
    }
    return Icons.auto_awesome_rounded;
  }

  Widget _tileCount(EurekaColors eu, int count) => Text(
    '$count',
    style: euMono(
      fontSize: count >= 100 ? 17 : 19,
      color: eu.textMid,
      fontWeight: FontWeight.w600,
    ),
  );
          ...
          children: [
            ...
            Flexible(
              child: SingleChildScrollView(
                child: Column(
                  children: [for (final t in widget.types) _typeRow(eu, t)],
                ),
              ),
            ),
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                child: Text(
                  _sel.isEmpty ? '选一个类型' : '导出 ${_sel.length} 类',
                  style: TextStyle(
      ...
      child: GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
              color: sel ? eu.brand.withValues(alpha: 0.6) : eu.border,
            ),
          ),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
