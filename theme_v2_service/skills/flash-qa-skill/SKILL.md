...
>
> **图文报告 / 复盘文档产物**(「帮我出一份消费报告」「把我的跑步做成复盘」)走 Report 主动流程:
> 在今日页的 Reka 容器选择「生成新报告」;历史报告也在同一容器查看。flash 里**不产报告**,收到这类请求时回一句**兜底指路**:
> 「想要一份图文报告的话,去今日页的 Reka 容器选择『生成新报告』。」(注意:用户只是随口问个数字/概况,如
> 「我这周花了多少」,仍是普通 qa,正常给短答 —— 别把简单查询也推去报告入口。)
>
> 用户说「保存/同步/沉淀到钉钉文档 / Notion / 外部系统」时,当前只给简短说明,
> **不得**声称已经执行,也不要编造工具结果。
...
5. **用户要一份图文报告/复盘文档产物**(「出一份消费报告」「把跑步做成复盘」)→ **不产报告**,只回兜底指路:「想要一份图文报告的话,去今日页的 Reka 容器选择『生成新报告』。」

**不要调** `tool_create_asset` / `tool_update_asset` 等任何写工具 —— 本 skill 无副作用。
**不要在答案里提及内部 skill 或实现机制** —— 答案只放纯内容。
...
**Output:**
```json
{"ok": true, "session_id": "<pass through>", "source_input_turn_id": "<pass through>", "answer": "想要一份图文报告的话,去今日页的 Reka 容器选择「生成新报告」。"}
```
