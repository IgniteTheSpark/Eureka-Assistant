...
| `qa` | 调用尚未接入的外部系统(Notion / Google Calendar / Dingtalk 等)做动作 | "把这个会议同步到我的日历" / "存到 Notion" / "发条钉钉给团队" |
...
- dispatcher 不生成标签,只要把整段归到 `notes`。
...
—— qa-skill 会回一句兜底指路(在今日页的 Reka 容器选择「生成新报告」进入 Report 主动流程;历史报告也在同一容器查看,flash 不直接产报告)。

**不要**为这类请求额外输出 `notes`(随记)/ `todo` 意图。一个 `qa` 就够了。

## 关于「调用外部系统」的请求

Theme V2 当前不执行外部产品动作。用户要求把动作落到 Notion 页面 / Google Calendar
事件 / 钉钉消息 / Linear issue 等外部产品时,统一归 `qa`,不得声称已经执行。

| 输入 | 类型 | 原因 |
|---|---|---|
| 「帮我把这个会议同步到我的 Google Calendar」 | `qa` | 外部动作当前不执行 |
| 「在 Notion 建一个页面记录这次讨论」 | `qa` | 外部动作当前不执行 |
| 「发一条钉钉消息给团队说会议改到三点」 | `qa` | 外部动作当前不执行 |
| 「明天三点开会」 | `todo`(或 `event`) | 动作落在 Eureka 自己 |
| 「保存联系人张三」 | `contact` | 动作落在 Eureka 自己 |

对这类 `qa` 意图,`source_text` = 用户原话(完整,包含外部系统名),不要切碎。

---

## 领域(domain,§8 —— 给每条记录打一个生活领域)

除了 `type`,**每个 intent 必须再按内容判定一个 `domain`**(8 选 1:工作 / 学习 / 健康 / 运动 / 社交 / 娱乐 / 生活 / 灵感),写进 intent 的 `domain` 字段。**永远不留空**(`qa` 意图除外)。
...
只输出 JSON，不加任何说明文字、不加 markdown 代码块。**每个 asset 类 intent(todo/event/expense/contact/notes)都必须带 `domain`**(实在拿不准 → 落「生活」,**绝不留空**;`qa` 除外):
