...
class QwenStreamingAsrProvider:
    """One Alibaba Qwen connection per Eureka voice-input session."""
    ...
    async def start(self) -> None:
        if self._started:
            raise StreamingAsrProviderError("invalid_state", "Qwen ASR already started")
        ...
        try:
            ...
            self._socket = (
                await connection if inspect.isawaitable(connection) else connection
            )
            ...
            if event != "task-started":
                raise StreamingAsrProviderError(
                    "connection_failed", "Qwen ASR did not become ready"
                )
            self._started = True
            ...
            await self._close_once()
            raise StreamingAsrProviderError(
                "connection_failed", "Qwen ASR connection failed"
            ) from exc

    async def send_audio(self, frame: bytes) -> None:
        if not self._started or self._terminal or self._cancelled or self._socket is None:
            raise StreamingAsrProviderError(
                "invalid_state", "Qwen ASR is not accepting audio"
            )
        try:
            ...
            raise StreamingAsrProviderError(
                "connection_lost", "Qwen ASR connection was lost"
            ) from exc
    ...
    async def finish(self) -> None:
        ...
        if not self._started or self._socket is None:
            raise StreamingAsrProviderError("invalid_state", "Qwen ASR is not started")
    ...
    async def events(self) -> AsyncIterator[ProviderTranscriptEvent]:
        ...
        try:
            while not self._terminal and not self._cancelled:
                ...
                self._terminal = True
                raise StreamingAsrProviderError(
                    "service_unavailable", "Qwen ASR returned an invalid event"
                )
        except StreamingAsrProviderError:
            raise
        except asyncio.CancelledError:
    ...
    def _transcript_event(
        ...
        if not isinstance(sentence, dict):
            raise StreamingAsrProviderError(
                "service_unavailable", "Qwen ASR returned an invalid result"
            )
        ...
        return ProviderTranscriptEvent(kind, self._sequence, text, self._duration_ms)
    ...
    def _decode(raw: Any) -> dict[str, Any]:
        if not isinstance(raw, str):
            raise StreamingAsrProviderError(
                "service_unavailable", "Qwen ASR returned a non-JSON event"
            )
        try:
            value = json.loads(raw)
        except (TypeError, ValueError) as exc:
            raise StreamingAsrProviderError(
                "service_unavailable", "Qwen ASR returned invalid JSON"
            ) from exc
    ...
    def _safe_failure() -> StreamingAsrProviderError:
        return StreamingAsrProviderError("service_unavailable", "Qwen ASR failed")
...
def _needs_space(left: str, right: str) -> bool:
    if not left or not right or left[-1].isspace() or right[0].isspace():
        return False
    return not (_CJK_RE.search(left) or _CJK_START_RE.search(right))
