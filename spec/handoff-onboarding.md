...
- ~~硬件配对 onboarding(随 §13 录音卡 SDK 落地再做)~~ → **✅ 已加(2026-06,jigong 录音卡 SDK 落地后)**:魔法时刻之后、进 app 之前加一个**可选的「连接录音卡」步骤**(`_Step.pairDevice`)—— narrative「你已体验随口记 → 连上录音卡,以后按一下就能记、不用打字 → [连接录音卡] / [先不用,直接开始]」,复用 jigong 的 `DevicePairingPage`,**可跳过**(没卡/想稍后的人绝不卡住,配对不是进门门槛)。
