...
| 6 | [06-synthesis-report.md](06-synthesis-report.md) | **合成·报告引擎**（设计中）：总结/升华/提案 dispatcher + 内容 skill + md→HTML 渲染 + GSAP/WebView + reports 实体 | 全栈 / AI / 前端 |
...
> DELETE/QUERY/SUMMARY/CHAT-ANSWER/CREATE-FROM-REPLY/CHAT），再决定调工具还是对话。
...
3. **SUMMARY 的「先 call 后说」铁律**：query → tool_render_report → 一句收尾，中间禁止预告性文字；
   HTML 只能进 `html` 参数、绝不进正文。这是 chat 体验最容易翻车处。
