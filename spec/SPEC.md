...
> **真值源（2026-06 起翻转）：产品 = Flutter app `mobile/` + `backend/`；旧 Web 参考代码已移除。**
...
>   标注「待实现」**，不代表已构建。
>
> 历史「分阶段重建」规划已在 2026-07 仓库清理中移除；以分章 `spec/` 为准。
...
| 戒指桌面端 | Python + Bleak + PyObjC（`ring-desktop/`） | macOS 手势、语音与触觉通知 |
...
权威客户端类型见 `mobile/lib/render/render_spec.dart`，服务端构建见
...
> 参考代码已移除。** 本章交互行为**冲突一律以 `mobile/` 为准**；
...
>
> 下文 CSS token 名称保留为历史设计语言；实现时以上述 Flutter 文件为准。
