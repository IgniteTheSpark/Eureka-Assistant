...
| 工具 | 签名（关键参数） | 落库 |
|---|---|---|
| `tool_create_todo` | `(title, content, due_date, session_id, source_input_turn_id, period?, occurred_at?)` | `assets`(skill=todo)——**待办专属 typed 工具**(薄封装 → create_asset；必须能承接 §4.5.0a 的模糊时段/具体时刻)。`title`=卡片/详情头部短标题；`content`=补充说明/子项/背景；简单单句可留空或等于 title |
| `tool_create_note` | `(content, title, tags, session_id, source_input_turn_id)` | `assets`(skill=notes/随记)——**随记专属 typed 工具**(tags 逗号分隔 ≤3) |
| `tool_create_asset` | `(user_skill_name, payload, session_id, source_input_turn_id, user_id, period?, occurred_at?)` | `assets`（+ `asset_fields` 索引）——**仅用于 expense + 自定义 skill**(内置走上面的 typed 工具) |
...
| `tool_add_event_attendee` | `(event_id, name, contact_id, role)` | `event_attendees`。`contact_id` 可空；手动 UI 优先绑定 contacts，agent 仅在唯一命中时绑定 |
...
> **领域(domain)增量(Layer A 已实现,横切章 §8)**:create 工具带 `domain` 参,落 asset 时按内容打(8 个生活领域;**服务端按链回落:域 → 技能 prior → 基线 prior → 「生活」兜底,永不 null**,产品决策 2026-06)→ 写 `assets.domain`;manual 创建/编辑由 §4 表单的 domain 选择器传入(可手动清空 → null)。详见 [§8 领域系统](08-domain-system.md)。
...
> **时段 / 时刻抽取（配合今日页时段分组 [§4.5.0a](04-frontend.md)，新增）**：sub-skill 落 asset 时，**只在用户明说时间时**填两个可空字段（[§2 §3.6](02-data-model.md)）：**`occurred_at`**（说了钟点，如「下午3点」→当天 15:00）、**`period`**（说了**模糊时段**「早上/下午/晚上」、或由 `occurred_at` 推）。**没说时间 → 两者皆 null、agent 不臆造**；今日页 / 流此时按 `created_at`（捕捉时刻）落到当下段（默认即捕捉时刻，`period` 只为模糊时段兜底）。**说了别的天 → 落对应日**：把相对 / 绝对日期（今天 / 昨天 / 前天 / 后天 / 周一 / 3号 / 上周五…）解析成具体日子、记录落那天（复用 captured_at-落对天）；一句话含多个日子（"昨天买衣服、前天打球"）→ 拆成多条各归各日；**那天若既无钟点又无时段 → 落那天「没说时间」组**（§4.5.0a，不拿今天的捕捉时刻兜底）；**闪念按捕捉日 +1**（原话今天能回看），产出卡归各自日。event 用既有 `start_at` 推 `period`。`tool_create_asset` 与 `tool_create_todo` 都必须支持 `period?` / `occurred_at?` 参，与 `domain?` 同形；一句话多意图各自抽。**内置 skill 与用户自定义 skill 同规**：`make_custom_skill_agent` 也必须注入这套规则，例如「昨天下午跑了5km」→ custom 跑步记录 asset `period=下午, occurred_at=null`，落昨天·下午「没具体时间」。**严禁把模糊时段 canonical 成假钟点**：`早上` 不是 08:00，`下午` 不是 15:00；只有「早上8点 / 下午3点」这类明确钟点才写 `occurred_at`。**todo 没说截止：`due_at` 留空**（落段靠 `created_at` 或 `period`；写死 due_at 会被 §14 误判逾期）。**（impl 注 2026-06）** `create_asset` 落 `occurred_at` 此前被 `_parse_occurred` 漏 `import datetime` 静默打断 → 历史 asset 的 `occurred_at` 几乎全是 None（落段一直靠 `due_date` / `created_at` 兜底）；已修（`mcp_server/tools.py`），新数据起 `occurred_at` 正常生效。同一入参也供「在这天记一笔」传 `created_at`（[§4.5.4](04-frontend.md)）。
...
**Flash 完成反馈（2026-07 新增）**：`_build_summary` 不再允许只回「已记录 N 项内容」这种机械统计。Flash 的卡片已经承担结构化结果展示，文字反馈的职责是让用户感觉 **REKA 听懂了这句话**。实现上应在结构化创建完成后，调用一个独立的 **Flash Reply Agent**，把 `source_text` + 已产出的 cards / derived_assets / pending 状态作为上下文，让 agent 自己理解用户刚记录了什么，并生成一句**短、温暖、具体**的回应。这里**不要用固定模板或按类型硬拼句子**；约束只用于边界：① 必须基于已产出内容，不编事实；② 不出现 asset_id / 工具名 / JSON /「我判断」等内部话；③ 不夸张、不卖萌、不连续感叹号；④ 多条时可以概括但要点到代表内容；⑤ 回复生成失败时 fallback 到保守但不机械的兜底（如「我帮你记好了。」/「这些我都帮你记好了。」）。Flash Reply Agent 失败不能影响卡片创建。
...
|---|---|---|---|
| **todo** | create/update/delete | `tool_create_todo` | `title/content` 拆分：title 是短动作标题（不含时间/领域），content 是详情正文（背景、子项、约束）；简单单句 content 可空/同 title。due_date：有时刻→ISO8601+08:00；只有日期→`"YYYY-MM-DD"`（**不猜时刻**）；无→null。`period`：只说「早上/下午/晚上」这类模糊时段时填；`occurred_at`：只有明确钟点才填。update/delete 先 query keyword |
| **event** | create/update/delete | `tool_create_event`（**非** create_asset） | Step 0 时段硬检查（缺时段直接拒绝、不自降级）；create 后抽取参会人：0 命中联系人→`name_raw` 裸名；1 命中→传 `contact_id` 绑定；2+ 重名→仍写 `name_raw`，等待用户在参会人 sheet 消歧。**不自动创建 contact** |
| **expense** | create/update/delete | `tool_create_asset(user_skill_name="expense")` | amount 必填；category 8 类推断；`date`(YYYY-MM-DD)；`period`/`occurred_at` 按 §4.5.0a 抽取。**不再生成 `at` 的早8/中12/下15/晚19 canonical 假时间**；历史 `at` 仅作兼容读取。 |
...
- **回执忠实**:agent 文案 = flash 的 `summary`（**真实覆盖全部产出**,但不是机械「已记录 N 项内容」;按 §1.3 Step3 生成温暖具体的一句话）；卡片**全落** `messages.cards`(reload 经 `CardsPart` 渲染)+ **实时**经一条合成 `tool_result`(`_group_cards` 把已 tag 的卡片重组成 assets/events/contacts/tasks 数组,前端现成 `extractCards` 全解析全渲染,**零前端改动**)。批量回合同样是 durable turn。
...
**外部 section 何时出现记录**：资产库「外部」只展示 `external_ref` 资产，也就是
`tool_create_task` 这条异步、可追踪管线产生的占位/完成引用。`use_connected_app`
即使成功执行了钉钉日历/待办的创建或修改，也只把结果作为本回合文字回执返回，**不会**
额外创建 `external_ref`，因此不会出现在资产库「外部」section。

**强制路由边界（2026-07 修正）**：凡是用户要求把 **Eureka 已有对象 / 本 session 刚记录的
对象 / 上一条回答内容** 写入外部系统，并使用「同步 / 放进 / 放到 / 加到 / 记到 / 存到 /
发到」这类导出动词，都必须走 `tool_create_task`。例：`可以帮我把小型讨论会的日程和饭局
放进钉钉吗`、`把 Eureka 的代办同步到钉钉待办`。这类请求不能走 `use_connected_app`，
即使后者也能在钉钉侧创建成功，因为它不会留下 `external_ref`。实现上
`agents.assistant._requires_tracked_external_task()` 会在 `use_connected_app` 入口拒绝这类请求，
迫使 Assistant 改调 `tool_create_task`。

**只读查询卡过滤**：Assistant 可能为了理解「把小型讨论会和饭局放进钉钉」先查本地
`event/todo` 来解析指代。`tool_query_*` / `tool_get_*` 及其无 `tool_` 前缀别名
（如 `query_event` / `get_event`）都属于临时查询视图，**只可实时折叠展示，不可写入
`Message.cards`**；否则会把本地 reference 卡误当成“同步结果”回放。
...
**先上 evals(已做)再动这些**;别在抽公共层之前堆 pet/game(§7/§9)。历史 review
目录已在 2026-07 仓库清理中移除；可执行回归真值保留在 `backend/evals/`。
