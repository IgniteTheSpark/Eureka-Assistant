...
**落点(设计中):** opener + starters 可随 `POST /api/sessions`(subject 锚定 get-or-create)的响应一并返回(L0/L1 服务端现算,便宜),或一个轻端点 `GET /api/sessions/{id}/opening-hint`;前端在 `ChatPage` 空态渲染(§4)。`session_subject_hint`/`session_assets_hint` 已注入(§1.5),L2 直接复用。
...
- `actions: "check"` 纪律：只有真正状态化的 skill（todo/打卡/review）才加，measurement/log 类
  （跑步/读书/记账）不加。
- **`chat_starters`（设计中，喂 [§1.5.1](#151-资产锚定会话的开场-hint设计中) 开场 hint 的 L0）**：2-3 条**起聊文案**，贴该技能的性质
