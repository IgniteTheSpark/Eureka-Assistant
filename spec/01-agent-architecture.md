...
| 工具 | 签名（关键参数） | 落库 |
|---|---|---|
| `tool_create_todo` | `(content, due_date, session_id, source_input_turn_id, period?, occurred_at?)` | `assets`(skill=todo)——**待办专属 typed 工具**(薄封装 → create_asset；必须能承接 §4.5.0a 的模糊时段/具体时刻) |
...
|---|---|---|---|
| **todo** | create/update/delete | `tool_create_todo` | due_date：有时刻→ISO8601+08:00；只有日期→`"YYYY-MM-DD"`（**不猜时刻**）；无→null。`period`：只说「早上/下午/晚上」这类模糊时段时填；`occurred_at`：只有明确钟点才填。update/delete 先 query keyword |
