...
- **lite 取图(打开提速 · 2026-06 · 后端已上线 / app 待接 jigong)**:那张内联 base64 配图把报告 `html` 撑到 **~2.4MB**(实测 Seedream 出图比上面 0.5–0.9MB 估值更大,单篇 base64 ≈ 2.4MB,真正 markup 仅 ~25KB)。应用内打开要跨境传整坨 → 慢(DB 读仅 9ms,瓶颈是 payload)。**存储不变**:`reports.html` 仍存自包含 base64 → **分享/导出的离线自包含不受影响**(§6.5/§6.6.1)。**应用内打开/生成**走 lite:`GET /api/reports/{id}?lite=1` 与 `generate` 的 `lite:true` 把内联 base64 剥成 `<img src="/api/reports/{id}/image/N">`(payload **~2.4MB→~25KB**,99%↓),配图改由 **`GET /api/reports/{id}/image/{idx}`** 按需取(从存储 html 抽第 N 张解码吐字节,**免 Bearer**〔凭不可猜的报告 UUID〕+ `immutable` 长缓存)。**向后兼容**:lite 全 opt-in,默认行为不变。**app 端(待 jigong)**:viewer 用 `lite=1` + `loadHtmlString(baseUrl=<api origin>)` 让根相对图 URL 解析;**分享必须取非 lite 自包含版**(lite html 的图是 URL,离线会裂)。落点:`api/reports.py`(`_to_lite_html` / `report_image` / `get_report?lite` / `generate.lite`)。
