...
**asset item 额外带的展示字段（驱动今日页 / 流 / 月 段视图 + 卡片，[§4.5.0a](04-frontend.md)）**：`period`（模糊时段，落段兜底）· `has_clock_time`（`occurred_at` 是否 set → 段内是否显 `HH:MM`）· **`domain`（✅ 2026-06 新增 = `assets.domain`，§8 生活领域 → 流/月卡片右上角领域 tag；移动端 `TimelineItem.domain` 解析）**。
