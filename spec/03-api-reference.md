...
| POST | `/api/assets` | `CreateAssetRequest{user_skill_name, payload, session_id, source_input_turn_id, domain?, period?, occurred_at?, created_at?}` | → `tool_create_asset`。`domain` 省略 → 服务端回落 技能 prior → 基线 prior → **「生活」兜底,永不 null**（§8.2）。`period?`/`occurred_at?` = §4.5.0a 落段真值;`created_at?` =「在这天记一笔」把记录锚到某天（[§4.5.4](04-frontend.md)） |
...
**asset item 额外带的展示字段（驱动今日页 / 流 / 月 段视图 + 卡片，[§4.5.0a](04-frontend.md)）**：`period`（模糊时段，落段兜底）· `has_clock_time`（`occurred_at` 是否 set → 段内是否显 `HH:MM`）· **`domain`（✅ 2026-06 新增 = `assets.domain`，§8 生活领域 → 流/月卡片**右对齐领域小色点**(taste-pass 后从色块 tag 改成点)；移动端 `TimelineItem.domain` 解析）**。
