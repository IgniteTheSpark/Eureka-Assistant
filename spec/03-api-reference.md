...
> **百智 OAuth 登录(✅ B1 已实现,[§13.1](13-baizhi-integration.md) + [`handoff-baizhi-oauth.md`](handoffs/handoff-baizhi-oauth.md))**:以百智作 IdP 的登录 —— bridge→换 token→映射到 Eureka `user_id`→**仍签发 Eureka 自己的 JWT**(每请求鉴权不变);百智真实 token 单独加密存,供调百智 MCP/API。
...
- `cards` 由 pipeline Step 3 纯 Python 聚合（event/task/contact/pending_contact 特殊分支 + 通用
  render_spec 路径）。
- `reply` 只放 QA/问答型内容；结构化记录成功后的用户可见确认句放 `summary`。`summary` 必须是**针对本次输入和产出卡的温暖短句**，不要返回机械的「已记录 N 项内容」。示例：`早上吃包子花了8` →「包子这笔 8 块我帮你记好了，放在今天上午。」；`下午要开一个会` →「下午的会我先帮你记着了，还没具体到几点。」；多条 →「都记好啦：早饭、咖啡两笔账，还有下午的会。」。
...
| POST | `/api/assets` | `CreateAssetRequest{user_skill_name, payload, session_id, source_input_turn_id, domain?, period?, occurred_at?, created_at?}` | → `tool_create_asset`。`domain` 省略 → 服务端回落 技能 prior → 基线 prior → **「生活」兜底,永不 null**（§8.2）。`period?`/`occurred_at?` = §4.5.0a 落段真值;`created_at?` =「在这天记一笔」把记录锚到某天（[§4.5.4](04-frontend.md)）。`todo` 在写入边界会把误放进 payload 的 `period/occurred_at` 提升为 asset 元数据并从 payload 删除。 |
...
`_serialize_asset` 返回：`{id, user_skill_name, payload, domain, period, occurred_at, session_id, source_input_turn_id, created_at}`。
...
| PATCH | `/api/events/{id}/attendees/{attendee_id}` | `AttendeePatch{name?, contact_id?, role?}` | 更新参会人；用于把裸名绑定到联系人、改角色、改显示名 |
| DELETE | `/api/events/{id}/attendees/{attendee_id}` | — | 移除参会人；不删除联系人真身 |
...
**参会人选择 / 联系人搜索契约（2026-07）**：
- `contact_id=null` 且 `name_raw` 非空是可长期保留的合法状态，不等同于“待确认”，也不要求用户后续必须处理。
- `GET /api/contacts?q=<keyword>&limit=20` 是参会人 sheet 的默认搜索源。`q` 做轻量模糊筛选：至少匹配 `name/company/title/phone/email` 的 contains（大小写不敏感；中文直接 contains）。
- 选择已有联系人 → `POST /api/events/{id}/attendees {contact_id, name: contact.name, role:"attendee"}`；后端写 `contact_id`，`name_raw` 可同步存一份当时显示名用于容错。
- 搜不到人 → sheet 显示「新增联系人」按钮；点开复用 `ContactForm` 创建名片，成功后自动把新 `contact_id` 加到当前 event attendees。
- 对已有裸名参会人执行「关联」时，前端以其 `name_raw` 预填联系人搜索；选中联系人后通过 `PATCH /api/events/{id}/attendees/{attendee_id}` 原位写入 `contact_id`，不新增第二条参会人记录。若搜索无结果，新建联系人表单继续预填当前搜索词；创建成功后仍 PATCH 原参会人。
- 重名由前端列表解决：同名候选必须显示副信息（公司/职位/电话片段），用户点哪条就绑哪条。后端/agent 不做“取第一条”的隐式选择。
- GET `/api/events/{id}` 返回 attendees 时，每项建议形如 `{id, contact_id, name_raw, display_name, role, is_resolved, contact_summary}`。
...
|---|---|
| event | `start_at` |
| todo | `occurred_at` \|\| `due_date` \|\| `created_at`（`period` 只影响段内归位,不制造钟点） |
| expense | `occurred_at` \|\| `date` \|\| `created_at`（legacy `at` 仅兼容旧数据;新 skill 不再把「早上/下午」写成 08:00/15:00） |
...
**asset item 额外带的展示字段（驱动今日页 / 流 / 月 段视图 + 卡片，[§4.5.0a](04-frontend.md)）**：`period`（模糊时段，落段兜底）· `has_clock_time`（`occurred_at` 是否 set → 段内是否显 `HH:MM`）· `has_scheduled_time`（todo 是否具有用户明确给出的具体时刻：`occurred_at` 或带 `T` 的 `due_date`；**绝不由 `created_at/effective_at` 推断**）· **`domain`（✅ 2026-06 新增 = `assets.domain`，§8 生活领域 → 流/月卡片**右对齐领域小色点**(taste-pass 后从色块 tag 改成点)；移动端 `TimelineItem.domain` 解析）**。

**回归约束（2026-07 验证补充）**：`period` 不能被服务端或 skill 转写成默认钟点。`早上吃包子花了8` 返回的 asset 应是 `period="上午"`, `has_clock_time=false`；`早上8点吃包子花了8` 才是 `has_clock_time=true`。`下午要开一个会` 应能通过 typed todo 写入 `period="下午"` 且不显示时间前缀。
...
| POST | `/api/reports/{id}/actions` | 一键 `+ 待办`:body `{title}` → **幂等**建 todo(payload 写 `title` + `content` 兜底为同一动作,并写 `assets.source_report_id` 列 + payload `source_report_title` 溯源);已建过返回既有 id(`created:false`) |
