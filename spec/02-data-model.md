...
| `period` | String(8) | nullable | **模糊时段（今日页时段分组，迁移待定）** ∈ `{凌晨,上午,中午,下午,晚上}` 或 null。**只在用户明说了模糊时段**（「早上/下午…」）时填，用来兜底归段；没说则 null，所属段由有效时刻推（见下）。agent 仅在用户明说时填、不臆造；**不得把「早上/下午」转成 08:00/15:00**。见 [§4.5.0a](04-frontend.md)。 |
| `occurred_at` | TS | nullable | **内容指向的精确时刻（迁移待定）**。仅当用户**说了钟点**（「下午3点」→当天 15:00）才填；否则 null。**≠ `created_at`（捕捉时刻）**。asset 有效时刻优先读 `occurred_at`，再读各 skill anchor（todo `due_date`、expense `date`、自定义 `timeline_anchor`），最后 `created_at`。**啥时间都没说**才回落捕捉时刻；**只说了模糊时段**（仅 `period`）的**不显示捕捉时刻**、进段内「没具体时间」组（[§4.5.0a](04-frontend.md)）。 |
| `created_at` | TS | | |
...
| `expense` | horizontal | 💰 | green | amount (currency) | description | amount(numeric), category(enum), date(date), merchant(text) | edit |
...
- `expense.payload` 新写入只保留 `date`（仅日期）等业务字段；精确钟点走 asset 级 `occurred_at`，模糊时段走 asset 级 `period`。历史 `at` 仅兼容旧数据读取，sub-skill 不再生成。
