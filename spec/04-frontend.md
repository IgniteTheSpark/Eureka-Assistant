...
- **编辑 = 全屏页 + markdown 编辑器(✅ Flutter `AssetEditPage`,对齐设计稿)**:原 Flutter 实现是「详情 sheet 上**再叠一个底部 sheet**」—— 输入大段内容时太憋。改为 **push 一个全屏编辑页**(appBar:取消 + 类型 caps + 保存):
  ...
  - **todo 特例(2026-07)**:`todo` 统一按 `title + due_date + content` 展示和编辑，且顺序固定。`title` 只用于卡片/详情头部；`content` 作为详情正文块；`due_date` 作为「截止时间」信息/meta,不再挤在头部 subtitle。历史 todo 只有 `content` 时,前端用 content 兜底 title；若 `content == title`,详情页不重复渲染内容块。todo 是系统内置严格契约：编辑页不显示 `status/period/occurred_at/domain`，也不并入历史 payload 的 orphan 字段；所有入口都先 hydrate canonical asset 再进入同一个 `AssetEditPage`。
  - **编辑按 skill 完整 schema 渲染(✅ 关键,修字段不一致)**:除系统内置 todo 外，控件来自 **`RenderSpec.schemaFields`(payload_schema 全字段,声明序)∪ payload 里的 orphan 字段**(如 skill 重生成后遗留的旧 `note`)——不再只渲染「该 asset 恰好抓到的字段」。**同一 skill 的每条 asset 编辑结构一致**(空字段也显示、可补全);保存时**新空字段不入 patch**。(根因:同一 reading_notes skill 被重生成过,schema 从 `note` 漂成 `key_insights`,旧 asset 留 orphan;三条读书笔记字段集各异。)
...
- **范围 = 可改领域的 asset-backed 表单**（`AssetEditPage`：随记 / expense / 自定义,**create 与 edit 同款**；todo 为固定三字段表单，不显示领域选择器）。选择器在表单末尾，
...
### 4.4.3b Event 参会人选择（2026-07）

**目标**：event 卡片和详情能表达“这场会和谁有关”，但不要求头像。参会人优先来自用户已有名片；没有名片时允许新建；AI 输入中提到的人名先安全落位，不乱绑重名。

**入口**：
- `EventForm` 新增「参会人」区，展示已选 attendees chip/list：`display_name` + 可选副行（公司/职位）。
- `EventForm` 与日程详情页都展示完整参会人列表。裸名参会人显示弱提示 `未关联联系人` 和可选操作 `关联`；不使用警告色、角标或强制处理流程。
- 点「添加参会人」打开底部 sheet：标题「选择参会人」，顶部搜索框，下面是「最近联系人」+「全部联系人/搜索结果」。
- sheet 支持多选；底部固定栏显示 `已选` 小圆点/名字缩略 + `保存(N)`。

**搜索 / 新增**：
- 默认从 `/api/contacts?q=<keyword>&limit=20` 搜用户名片。前端做即时输入防抖；后端做简单模糊匹配。
- 无结果或用户确认不是同一人时，显示「新增联系人」按钮；点开复用 `ContactForm`（不需要头像），创建成功后自动选中并添加到 event。
- 从裸名参会人的 `关联` 入口打开 sheet 时，自动以其 `name_raw` 预填并触发搜索；若继续新建联系人，`ContactForm` 默认以当前搜索词预填姓名。
- 重名必须靠副信息消歧：同名列表显示 `company/title/phone` 片段；没有副信息也要显示为两条独立候选，用户手选。

**写入**：
- 选择已有联系人：`POST /api/events/{event_id}/attendees {contact_id, name: contact.name, role:"attendee"}`。
- 新建联系人后：先 `POST /api/contacts`，再 `POST /api/events/{event_id}/attendees`。
- 移除 attendee 只删 `event_attendees` 行，不删除联系人。
- 对闪念创建出的裸名 attendee，用户在 sheet 中选择联系人后用 `PATCH /api/events/{id}/attendees/{attendee_id} {contact_id, name}` 绑定。
- 用户可以永久忽略关联操作；裸名参会人不影响日程保存、卡片展示和日历流使用。

**EventCard 摘要展示**：
- 完整/通用 EventCard（资产库、对话内卡片、详情/编辑预览）的 meta 行固定走短摘要：`14:00–15:00 · 会议室 · Alex +3`。
- 拼接顺序：时间范围 → location（有则显示）→ attendees 摘要（有则显示）。
- attendees 摘要：取第一位 `display_name`；总数 > 1 时显示 `+N`，其中 `N = total - 1`。没有头像，不做 avatar stack。
- 若第一位是未关联裸名，照样显示 `name_raw`；详情/编辑里显示 `未关联联系人` 和可选的 `关联` 操作。
- EventCard 不区分参会人是否已关联联系人，不展示 `?`、`?N`、待确认角标或其他额外状态标识。
- **日历流例外**：流/月的紧凑 event item 继续遵循 §4.5.0a，只展示单行 `[时间] [类型 emoji] 标题`；不把时间范围/location/attendees 的第二行 meta 塞进日历流。点开详情后再展示完整摘要与参会人。
...
> - **年月常驻(2026-07 修复)**:切换器下方、流内容上沿固定显示当前视口的 `YYYY年M月`，由最上方可见日期驱动；跨月滚动时淡入切换。该 header 必须占用独立布局高度，列表放在其下方的独立滚动 viewport 中，禁止用覆盖列表的 `Positioned` overlay 或仅靠列表 top padding，避免滚动时年月遮住日期。不能只依赖月初 row 的竖排锚点，因为默认定位到月中“今天”时月初已在屏幕外。原月初竖排锚点继续保留，负责跨月节奏。
> - **左列 = 日期 + 闪念**:周缩写(mono)+ 大日号(今天蓝);`⚡N` pill **在日号下方**。左列作为**一个整体 sticky 跟随滚动**(钉在当天顶部、滚到当天底部停、被下一天推走);**钉住后用 live scroll offset 驱动 `Transform`(冻结 pin-anchor),不再每帧 `localToGlobal` → 0% jank、不抖**。程序化大跨度定位（如「回到今天」）后必须以当前 `scrollOffset + tileTop` 校验并重同步过期 pin-anchor，首帧即与右侧 day 容器顶边平齐，不能等用户再次滑动才归位。**无「今天蓝点」**(被当 bug 去掉,今天靠日号蓝表示)。
...
> **待办 `due_date` / `period` 注意**：todo 没说具体截止钟点 → **别把 `due_date` 写成捕捉时刻**，否则 §14.6 立刻判它"逾期·拖了 N 天"。只说「下午/晚上」时,typed todo 必须写 `period=下午/晚上`、`occurred_at=null`、不写假 `due_date=15:00`；完全没说时间时，`created_at` 只用于归属/排序，UI 必须把它放入「待安排」，不能显示成捕捉时刻的日程。
...
- **B** 流内容上沿常驻当前视口 `YYYY年M月`，跨月跟随切换；每月首行左 rail 仍显「2026 / X月」竖排锚点，当月 brand 蓝 + 辉光。
...
完整/通用 EventCard（资产库、对话内卡片、详情/编辑预览）的第二行 meta 统一为短摘要：

```text
14:00–15:00 · 会议室 · Alex +3
```

拼接规则：时间范围必显；location 有则追加；attendees 有则追加“第一位参会人 + 剩余人数”。完整参会人交互见 §4.4.3b。

**日历流不套用这条第二行规则**：流/月中的 event item 保持 §4.5.0a 定义的单行 `[时间] [类型 emoji] 标题`，meta 留在详情/完整卡片中展示。
