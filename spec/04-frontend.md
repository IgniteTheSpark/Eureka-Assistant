...
> - **焦点卡 = 全局统一 `SkillCard`（`'horizontal'`）**——与资产库/日历同款（event → `card_type:'event'`+synthesizeSpec；todo → registry spec），不再自绘焦点卡（去掉自定义倒计时/进度条/完成 pill）。卡 `IgnorePointer` 包住（自身 tap/勾选/左滑删除不与卡叠手势打架），**点击由卡叠层接管 → 全局详情 sheet**（完成/讨论/编辑/删除都在 sheet 里）。`ChainItem.card` 在 `_loadChain` 预构建。无时间待办仍是勾选清单（保留 R7 完成不消失·划线）。
> - **物理 = forge2d（Box2D ≈ 用户的 Matter.js 参考）**（`bubble_physics.dart`）：球 = 动态圆（弹性 .45、低摩擦=可滚、`bullet` 防穿透）；**底界 = ∧ tent**——apex 在 dock 顶中心、两条 static edge 发散到底部两角 + 左右壁 + 天花，球**滑向两侧、永不压上或钻到 dock 下方**（取代会把球向下弹的 AABB）；拖拽 = velocity-chase（软跟手 + 抛）；倾斜重力；Box2D 原生 sleeping 省电（全睡停 ticker）。px↔米 scale 40。
