...
>
> **DayDetail:** 「非日程」= `DayRender`(同段逻辑);**「日程」24h 网格 + 待办落格 = Part B(✅ 2026-06 已落地)**:事件 + 有时刻待办进网格、待办带 ○ 勾选、同点 N 个 = 计数 chip(点开手风琴)、结果记录 = 记录容器、默认镜头 =「日程」。**视觉精修(taste-pass)**:**记录段 / 日程段 = 两个对等 section**(记录的类型 tab 在容器内)、**全天 + 待安排 = 网格顶部左右并列的同款托盘**(全天**正常行、非 pill**)。详见 [`handoff-calendar-design.md`](handoff-calendar-design.md) §B(含 taste-pass 块)/ 线框 `日历改版线框.dc.html`。

### 4.5.0 今日页（`TodayPage`，首页 tab0 = landing：① Next Action ② Recorded）—— 重定义 2026-06（hifi prototype 已出）

> **今日页 = app 首页 / landing**：两块面板浮在一个**全屏物理气泡场**之上，Reka 浮球在最上层。**不是「今天的段视图」**（段视图是回看 = DayDetail / 流，§4.5.0a / §4.5.4）。**hifi 视觉真值 = 用户 prototype（`design_handoff_today_page` README）+ [handoff-calendar-design §E](handoff-calendar-design.md)**；本节是逻辑 / 数据 / 决策。
> **nav（2026-06 改）**：底栏 = **今日 / 日历 / 资产**（日历回归 tab）；**我的岛 → Reka 浮球雷达菜单**（§4.1）。

> **✅ 已落地 + 设计评审轮 2026-06-22**（branch `feat/today-page-landing`；代码：`pages/today_page.dart` + `today/{today_data,next_action,bubble_pool,bubble_physics,dashboard,charts,today_summary,today_palette}.dart`；纯逻辑单测 15 个，物理交给 forge2d）。决策 / 与原型的偏差（含两轮真机评审的修正）：
> - **亮 / 暗两套**（`today_palette.dart` 跟随 themeMode）：**暗 = 原型 atmosphere**（逐字），**亮 = 暖色、沿用 app eu 令牌**，landing 与 日历/资产 一致。全局头仅暗模式 `onDark`（亮模式今日页本身是浅色）。
> - **记录详情 = 复用全局 `showAssetDetail`**：球 + dashboard 最新行都走它（`buildCard` + render_spec 构造 CardData，主题感知），**不自建 sheet**（评审轮把早先的暗色只读 sheet 撤掉了，要求和全局一致）。
> - **接下来 = Tinder 卡叠**：真 stack 叠层（背后 1–2 张缩放偏移壳）+ 拖动跟手 + 旋转 + 过阈/快滑飞出、否则回弹（取代原型 C-fan）。`AnimationController` 驱动。**滑到最后一张之后 = 「暂时没有了」末态卡 + ↺ 回到开头**（右滑回看上一条）。
> - **焦点卡 = 大卡片嵌套全局 `SkillCard`（`'horizontal'`，与资产库/日历同款）+ 实时倒计时 footer**（event → `card_type:'event'`+synthesizeSpec；todo → registry spec）。外层大卡 **opaque**（cardTop/cardBottom）→ 不透出下层（修早先「能看穿到下层卡」像未完成态）；`SkillCard` 用 `IgnorePointer` 包住（自身 tap/勾选/左滑删除不与卡叠手势打架），**点击由卡叠层接管 → 全局详情 sheet**（完成/讨论/编辑/删除都在 sheet 里）。倒计时（`fmtCountdown` + 1s tick）：event 还剩/进行中/X 后开始，timed todo X 后到期。卡叠**壳做成实心卡**（不再半透明）。`ChainItem.card` 在 `_loadChain` 预构建。无时间待办仍是勾选清单（保留 R7 完成不消失·划线）。
> - **物理 = forge2d（Box2D ≈ 用户的 Matter.js 参考）**（`bubble_physics.dart`）：球 = 动态圆（弹性 .45、低摩擦=可滚、`bullet` 防穿透）；**闭合 box（floor + 左右壁 + 天花）+ dock = 实心 static box collider**（下延到屏底、堵住浮动 dock 下方 ~14px 缝）→ 球与 nav **真实物理碰撞**，堆在 dock 顶与两侧角、不压不钻（取代 ∧ tent / 更早会把球向下弹的 AABB）；**drop-in 落点在天花内侧（y>0）**，不在其上方（否则被天花 edge 挡住、卡在顶部 nav 下不来）；拖拽 = velocity-chase（软跟手 + 抛）；倾斜重力；Box2D 原生 sleeping 省电（全睡停 ticker）。px↔米 scale 40。
> - **重力 = 倾斜方向 + 恒定模长（≈20，world 单位）**，近水平兜底向下（平放不空池）；单 `Ticker`（每帧 `world.stepDt`）+ 加速度计仅「tab active + 前台」时跑，全睡即停。
> - **dashboard**：summary 最新一条可点 → 开全局 sheet **+ 点亮对应球**（其余球暗下 5s）；玫瑰图单组画整饼、legend 右对齐（去右侧大留白）；图表切换有动效（`AnimatedSwitcher`）。
> - **待办「有时刻」= due_date 带钟点**（ISO `T…`）→ 进 chain 倒计时；**待安排（无时刻）完成 = 画删除线保留**、不消失（done 仍在列表）；chain 事件按「结束」才移除（进行中仍是焦点卡 + 进度条）。
> - **球池 / 今天 N 颗 / 玫瑰图 = 今天所有捕获 = assets + 今天创建的 event + 名片(contacts)**（不再只 `/api/assets`；`_loadPool` 并发拉三源：assets/`created_*`、`/api/events?created_from/to`〔新增 `query_event` 按 `created_at` 过滤,区别于 `start_at` 日历过滤〕、`/api/contacts` 客户端按 `created_at` 过滤）。**无 domain 的处理**：event 表无 domain → **中性色气泡 + 按领域归「未分类」**（不造假领域）；名片表无 domain → **默认 社交**。filter/「全部·按类型」chart 本就按 type → event = 事件 slice、名片 = 名片 slice,与 domain 无关。
> - **气泡 glyph + dashboard 分类 icon/名 = 每个 skill 的 `render_spec.icon` + `display_name`**（经 `resolveMeta(type, skills)` 解析，`skills` = `loadToday` 里 `fetchSkills` 拉的注册表，与日历/资产库同源）—— **自定义 skill 也对**（running → 🏃 跑步训练，不再硬编码成 🎾/运动；内置类型仍由注册表给：待办 📋 / 日程 📅 / 记账 💰 / 名片 👤 / 随记 ✍️）；**解释性文案去除**（左右滑提示 / 🔔到点提醒你 / 空态副句）；**dock 立体**（分层阴影 + 顶亮玻璃渐变，两套都做）。
> - **后续（未做）**：早报 merge 进 Next Action（§14.6 gate 复用）；倾斜符号真机微调。

**Part 1 · Next Action（接下来要做的）—— 前向、动态**
- **Tinder 卡叠**（评审轮：取代原型 C-fan）：背后 1–2 张缩放偏移卡壳（可见叠层）+ 焦点卡（固定高，事件 / 待办不变形）。**拖动跟手 + 旋转**，过阈 / 快滑飞出切上 / 下一条、否则回弹；到点 / 完成自动前移。头部可折叠（`接下来` + `1/N` chainPos + caret）。
- 焦点卡：域色点 + 子标题（`事件 · 地点`）+ 时间；大标题。**事件卡** = **倒计时**（每秒 `⏳ N 分 M 秒后` / 进行中·还剩）+ 进度条 + `在日历看 ›`；**待办卡** = 备注 + `完成 ✓`。（解释性文案如 `🔔 到点提醒你` 已去除。）
- **🕒 无时间待办 N ▾**：展开 = 虚线容器，逐条 域色点 + 标题 + 圆勾选（下推内容）。
- **早报 merge 进这块**（[§14.6](14-proactive-reka.md)）：早报的「今日日程 / 待办」就是这条 chain 的今日内容；上午首开可展开更完整早报（昨日回顾 / 本周进度作开场或末尾一拍）。**不再独立晨报页**。
- **空态**：🌤️ +「今天还没有日程或待办」+ 一句邀请。

**Part 2 · Recorded（今天记了什么）= Dashboard + 气泡池 —— 按「创建 / 录入时刻」收今天的 asset**
- **关键区别（点睛）**：日历按 **effective time（发生时刻）**；**今日页按 `created_at`（录入时刻）** —— 今天建「明天4点上线」→ 今天池里有、日历在明天（同一 asset 两个镜头）。
- **闪念 = pill，不进池、不是记录类型**（产品定）：dashboard 头一颗 `⚡ N` pill = 今日闪念入口 → **当日闪念 session**；池子与类型 chips **都不含闪念**（池 = 闪念 / 手动**产出的结构化 asset**）。
- **气泡池（全屏后层、浮在面板之后、不与面板碰撞）**：一条记录 = 一颗气泡（**域色填充 + 类型 emoji glyph**；域色 = **§8 八色**，[§5.1](05-design-system.md)；域 = 填充色、类型 = glyph）。**点 → 记录详情 sheet；拖 → 抛；倾斜手机 → 重力跟随滚动；新记录从顶掉入**。物理：无壁盒（地板在 nav 上方、天花在顶、左右壁）+ nav 胶囊作 AABB 碰撞体 + 圆-圆碰撞 + 多趟松弛 + **休眠去抖**（g≈0.44 / 弹性≈0.22 / 阻尼≈0.97）。**空态**：🫧 +「今天还没有记录」。
- **Dashboard（可折叠；无记录整块隐藏）**：
  - 头：`今天 N 颗` + `⚡ N` 闪念 pill（见上）+ caret。
  - **类型筛选 chips**（横滑）：`全部 N` + 每个出现的类型一颗（📌待办 💰记账 👤名片 📝笔记 🎾运动 + 自定义技能；**无闪念 chip**）。点 chip → **同时筛池子 + 重算 summary + 重算图**。
  - **summary 速览条（不强行 generalize）**：自定义字段无法判断哪个数字是关键，所以**只特化少数已知类型，其余只显"今天最新一条"**：
    - **记账（特化）**：最新一笔（标题 + ¥）· 今天最大一笔（¥X）· **右侧大指标 = 今日总消费 `¥sum`**。
    - **其他所有类型（默认，含待办 / 名片 / 笔记 / 运动 / 自定义技能）**：**今天最新收录的那一条**（类型图标 + 标题 + 时间），**不算聚合**。
    - **全部**：今天共 N 条 + 最新一条预览。
    - 日后如有别的"已知聚合"类型再单独特化（如待办进度）；默认永远是"最新一条"，custom 技能也安全。
  - **注**：上面只是 summary 条；**图表（玫瑰 / 柱状 / 树图）不受影响** —— 它们按**条数**分组（类型 / 领域计数），不解读自定义数值，照常通用。
  - **图表**（玫瑰 / 柱状 / 树图，下方选择器 + 滑动切）：**drill —— `全部` 按类型分组、选某类型 按领域分组（§8 八域）**。
- **记录详情 sheet**：底部 sheet，域色圈 + 类型 glyph + 标题 + meta + 两 pill（类型、领域）。

**Reka = 全局悬浮球**（角落可微反应），**不占首页中心、不做仪表盘**。

**4 个边态**（prototype 暴露）：完整一天 / 只有日程·无记录（dashboard 隐、池空态）/ 只有记录·无日程（Next Action 空态）/ 全空·清晨。

**质感铁律**：**高级**（克制、材质 / 光 / 深度，不卡通玩具感）· **直观**（一眼看清 接下来 + 今天记了多少）· **有交互**（卡叠滑、图表切、气泡 点 / 拖 / 倾斜 / drop-in）。

> **数据（无新模型）**：Part 1 = 今日及临近事件（`start_at`）+ 到期待办，串 chain；Part 2 池 = **`created_at` 在今天**的 asset（≠ 闪念原始捕捉；闪念 pill = 今日 flash session 计数 → §4.5.0b）；点球 = 开 asset 详情；球色 = `assets.domain`（§8 八色）；summary / 图表的求和 · 完成度 · 领域分组都从这批 asset **现算**。
> **design's call（hifi 真值 = prototype README + [handoff §E](handoff-calendar-design.md)）**：扇形卡叠 + 倒计时；dashboard chips / summary / 三图表 + drill；气泡池物理 + 倾斜 + drop-in + 详情 sheet；两块整屏层级与高级质感。
...
> **待办 `due_at` 注意**：todo 没说截止 → 落段靠 `created_at` 即可，**别把 `due_at` 写成捕捉时刻**，否则 §14.6 立刻判它"逾期·拖了 N 天"。`due_at` 留空。
