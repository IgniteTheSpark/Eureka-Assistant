...
> **✅ 已落地 + 设计评审轮 2026-06-22**（branch `feat/today-page-landing`；代码：`pages/today_page.dart` + `today/{today_data,next_action,bubble_pool,bubble_physics,dashboard,charts,today_summary,today_palette}.dart`；纯逻辑单测 20 个）。决策 / 与原型的偏差（含真机评审轮的修正）：
...
> - **接下来 = Tinder 卡叠**：真 stack 叠层（背后 1–2 张缩放偏移壳）+ 拖动跟手 + 旋转 + 过阈/快滑飞出、否则回弹（取代原型 C-fan）。`AnimationController` 驱动。
> - **物理 = 自写 solver**（`bubble_physics.dart`，单测）；池上限 **50 球**；**休眠球作不可动锚点**（消除落定后持续抖动）；nav 胶囊 AABB 对齐真实 dock（≈180×52）。
