...
- **D** 空日 = 同色空间；`仅有事/全部` toggle（持久化 `eureka:schedule_show_empty`）控制空日是否折叠成
  ...
  - **空日(`_EmptyDayRow`)= 点选式快记,不常驻按钮**(用户:别在每个空日常驻「记一笔」)。默认空 tile;**点(日期或 tile)→ 选中**(`_selectedDay`,高亮)→ 露出「空闲 · **+ 在这天记一笔**」→ 点它 `showCreateMenu(presetDate: day)`;再点 tile 取消。**空日不进日视图**(没内容可看)。
  - **有内容的日(`_DayRow`)→ 点条目 = 开该条详情;点 tile 的空白处 / 日期 = push 该天 `DayDetailPage`(日视图)**。
...
- **FlashItemRow**：闪念在流里渲染成 ⚡ + 产出 breakdown「✅ 待办×2 · 👤 联系人×1」（`derived` 字段，
  自建 skill 经 registry 取 icon/label），点开捕捉 session。**只展示产出数量，不展示
  transcript/摘要**（全文在捕捉 session 里看）；无产出时退化为单字「闪念」。
