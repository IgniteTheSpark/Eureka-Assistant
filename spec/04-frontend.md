...
> - **卡片 = 带边框小卡片**(左侧时刻列 + 卡片:icon + 标题省略 + **领域 tag**)。**领域 tag 进流了**(下面 §4.5.0a「领域 chip 退详情卡、不进流」已作废):接 §8 domain —— **timeline item 新增 `domain` 字段**(后端 `core/timeline.py` asset item + 移动端 `TimelineItem.domain`),`DomainChip`/`domainColor` 配色。
...
> 2. 说了时段没说钟点(`period` 有、无钟点)→ 落该段、**沉到段尾、虚线边框(`_DashedRRect`)、不显时刻**(段内不再另起"没具体时间"子分隔)。
...
>
> **DayDetail:** 「非日程」= `DayRender`(同段逻辑);**「日程」24h 网格 + 待办落格 = Part B(✅ 2026-06 已落地:事件 + 有时刻待办进网格、待办带 ○ 勾选、同点 N 个 = 计数 chip、无时刻 = 待安排条、结果记录 = 记录·按类型容器;默认镜头 =「日程」)** —— 见 [`handoff-calendar-design.md`](handoff-calendar-design.md) §B / 线框 `日历改版线框.dc.html`。

### 4.5.0 今日页（`TodayPage`，首页 tab0 落点）—— 新增

> **dock 第一个 tab「今天」的落点从「CalendarPage 流·今天」改为本页 `TodayPage`**（§4.1 dock 标签不变，仍叫「今天」）。日历（流/月/年）**降级为回看 / 规划入口**：今日页右上 **📅** → `CalendarPage`（§4.5.1）。**底部仍 3 tab、Reka 仍只在浮球 + 我的岛**（经产品确认：Reka 主动内容收敛在 Reka 菜单，不挂今日页）。

**今日页 = 纯「今天内容」页**（不是 Reka 仪表盘），自上而下：

1. **晨报 hero（上午首开，可折叠）**：北京时间中午前当天首开 → 顶部展开 [§14.6](14-proactive-reka.md) 晨报 hero（早安 + 今日预览 + 昨日回顾 + 本周进度）；✕ /「开始今天」收起，当天不再展开。**晨报并入今日页** —— 不再是独立沉浸页（§14.6 已改）。三级 gating 不变（全新用户让位孵化、数据太薄跳过）。
2. **header 行**：日期（今天）+ 右上 **📅**（→ CalendarPage）。
3. **今天的「一天渲染」**（§4.5.0a，=「非日程」模式）：5 时段（凌晨/上午/中午/下午/晚上），装今天的事件 + 结构化记录 + 机会实体 + 今日到期待办；没说时间的按捕捉时刻落到当下段。**今日页默认 = 非日程，不是日程（网格）**；header 有 `⚡ 今天 N 条闪念` 入口（§4.5.0b）+「日程 / 非日程」toggle（§4.5.4），想看今天的精确日程一键切。

**今日页 vs 日历流 = 同一套「一天渲染」组件**：今日页只渲染**今天**（+ 晨报 hero）；日历流是历史 / 未来**逐日**复用同组件回看。不重复造轮子。

### 4.5.0a 「一天渲染」DayRender —— 5 时段 + 闪念 chip（新增）

一天内容的渲染组件，**今日页（今天）+ 日历流（每一天）共用**（=「非日程」模式 = 上午/下午/晚上 分段；「日程」模式 = 24h 网格在 DayDetail，§4.5.4），取代旧「按时间戳排成一条流」。**两处都折叠空段**（只画有内容的段，最轻；不画固定骨架）。
...
### 4.5.1 CalendarPage（流/月/年 —— 经今日页 📅 进入）
...
- **日视图 `DayDetailPage`（Flutter，全屏；web 是 `DayDetailSheet`）**：
  ...
  - **右下 `FloatingActionButton.extended「+ 在这天记一笔」**(2026-06 新增)→ `showCreateMenu(presetDate: 当天)`,新建字段预设到这天。**这是「往某天加一笔」的可靠入口**(不依赖流 tile 的留白)。
  - **一个 toggle、两个模式（沿用现有名称「日程 / 非日程」）**：
    ...
    - **「日程」= 24h 网格**（保留），日程向：事件按 `start_at`–`end_at` 画**时长块**（3–5 点 = 一个 2h 块）；全天事件走顶部「全天」条；无时刻捕捉收在**顶部类型 tab / 列表区**（"顶部 tab 区分 assets"）。`end_at` / 时长是「日程」独有、「非日程」丢掉的信息。"把这天当日程表看"。
      - **✅ Part B（2026-06 已落地)**:网格里**只放事件 + 有时刻待办**,结果记录收进「**记录 · 按类型**」定高容器。**版面上→下(用户定)= 记录容器(顶) → 「日程」section**(「全天」条 + 「待安排」条 + 24h 网格,**待安排是日程的一部分、在网格之上**)。**待办落格** = 有时刻小块(带 ○ 勾选框)/ 同点 N 个 →「**N 个待办 ▾**」计数 chip(点开就地展可勾小列表)/ 无时刻 →「**待安排**」条(≤3 + 共 N + 展开其余);**○ 点击直接完成**(PUT `payload_patch.status` + bumpData)。重叠走 `_eventColumns` 等分列。**待补(下轮 polish)**:点状待办撞长事件的"左半瘦 chip"形态(当前等分列、功能正确)。完整设计 + 落地状态 = [`handoff-calendar-design.md` §B](handoff-calendar-design.md)。
...
向导生成的 config **创建后还能改**:技能的**类目页**(`CategoryDetailPage`)appBar 加 `⚙ 卡片配置` 入口(任意 `userSkillId != null` 的技能)→ push **`SkillConfigPage`**:拉 `GET /api/skills` 取该技能当前 `render_spec` + `payload_schema` + `display_name`,塞进 **`SkillConfigForm`**(从向导 preview 步抽出的**可复用**配置表:**实时卡片预览** `CardPreview` + icon(≤2) + 显示名 + accent 7 色板 + 每字段 `主/副/信息/隐藏` 角色,逻辑与向导一致:主/副唯一、信息 ≤3)。保存 → **`PATCH /api/skills/{user_skill_id}{render_spec, display_name}`**(后端 `update_skill`,只改**展示**,不动 `payload_schema`/字段 key 这条数据契约)→ `ref.invalidate(renderSpecsProvider)`(全局卡片按新 icon/色/字段位重渲)+ `bumpData()`。预览样例值由 schema 合成(`_sampleFor`)。**注**:向导当前仍保留各自一份配置 UI;后续可让向导也用 `SkillConfigForm` 去重(暂为降风险未重构)。
