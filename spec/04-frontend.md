...
> **✅ 已落地 2026-06-22**（branch `feat/today-page-landing`；代码：`pages/today_page.dart` + `today/{today_data,next_action,bubble_pool,bubble_physics,dashboard,charts,today_summary}.dart`；纯逻辑单测 19 个）。落地时定下的决策 / 与原型的偏差：
> - **物理引擎 = 自写 solver**（`bubble_physics.dart`，单测）；池上限 **50 球**（真数仍显示在 dashboard 头）。
> - **省电三重 gating**：单 `Ticker` 仅在「tab active + app 前台 + 有球醒着」时跑，全睡即停；加速度计仅 active+前台时订阅。
> - **重力 = 倾斜方向 + 恒定模长（≈.44）**，近水平（平放桌面）兜底**向下**——纯加速度计会让平放时气泡飘到顶（空池观感差），故偏离原型的纯 tilt。
> - **记录详情 = 暗色只读 sheet（`#161b22`，原型款）**，**不复用 light 的 `showAssetDetail`**（暗页上 light sheet 割裂）；编辑走 资产 / 日历（池 = 一瞥）。
> - **全局头 tab0 暗色**（`GlobalHeaderBar(onDark)`）——否则 light bar 压在暗页上割裂。
> - **待办「有时刻」= due_date 带钟点**（ISO `T…` 段）→ 进 chain 倒计时；纯日期 due → 无时间待办。（`has_clock_time` 是 occurred_at-based，pending 待办没有。）
> - **chain 事件按「结束」才移除**——进行中的会议仍是焦点卡（带进度条），非一过 start 就消失。
> - **气泡 glyph = app 既有**（待办 📋 / 随记 ✍️ / 记账 💰 / 名片 👤），非原型的 📌/📝（全局一致）。
> - **后续（未做）**：早报 merge 进 Next Action（§14.6 gate 复用）；倾斜符号真机微调；`/design-review` 终轮质感打磨。

**Part 1 · Next Action（接下来要做的）—— 前向、动态**
- **扇形卡叠（C-fan）**：2 张虚化偏转 peek 卡 + 1 张焦点卡；**焦点卡固定高**（事件 / 待办不变形）。**左右滑**切上 / 下一条；到点 / 完成自动前移。头部可折叠（`接下来` + `1/3` chainPos + caret）。
- 焦点卡：域色点 + 子标题（`事件 · 地点`）+ 时间；大标题。**事件卡** = **倒计时**（每秒 `⏳ N 分 M 秒后`）+ 进度条 + `🔔 到点提醒你` + `在日历看 ›`；**待办卡** = 备注 + `完成 ✓`。
