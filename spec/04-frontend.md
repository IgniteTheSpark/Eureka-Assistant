...
>
> **DayDetail:** 「非日程」= `DayRender`(同段逻辑);**「日程」24h 网格 + 待办落格 = Part B(待实现,下一步)** —— 见 [`handoff-calendar-design.md`](handoff-calendar-design.md) §B / 线框 `日历改版线框.dc.html`。
...
- **日视图 `DayDetailPage`（Flutter，全屏；web 是 `DayDetailSheet`）**：
  ...
  - **一个 toggle、两个模式（沿用现有名称「日程 / 非日程」）**：
    ...
    - **「日程」= 24h 网格**（保留），日程向：事件按 `start_at`–`end_at` 画**时长块**（3–5 点 = 一个 2h 块）；全天事件走顶部「全天」条；无时刻捕捉收在**顶部类型 tab / 列表区**（"顶部 tab 区分 assets"）。`end_at` / 时长是「日程」独有、「非日程」丢掉的信息。"把这天当日程表看"。
      - **⏳ Part B（待实现,下一步）**:网格里**只放事件 + 待办**,结果记录改收进顶部「记录·按类型」定高容器;**待办也落格**(有时刻小块 / 撞长事件瘦 chip / 同点 N 个计数 chip / 无时刻「待安排」条)+ **重叠分列规则**。完整设计 = [`handoff-calendar-design.md` §B](handoff-calendar-design.md)。
