...
## 14.5a offer 的两条到达路径：PUSH(peek)与 PULL(今日页 Reka Offer 屏)（产品定 2026-06）

> 今日页【Reka Offer】屏([§4.5.0](04-frontend.md))是 §14 offer 系统的 **PULL 落地面**。同一批 offer 现在有两条到达路径,**共享同一份 offer 与状态、不产生两条**:

| | **PUSH · peek** | **PULL · 今日页 Reka Offer 屏** |
|---|---|---|
| 谁发起 | Reka 主动弹(heartbeat,§14.1-14.4) | 用户**主动来翻** |
| 时机/算法 | 心跳扫到 → 浮球 peek 气泡 | **打开即时现算**当前所有成立的 offer(随记/灵感攒够→整理、记账攒够→消费总结、习惯断签、逾期、会前调研) |
| 护栏 | **全套**(≤2/天、置信门、安静时段 22-08、退避,§14.8)—— 因为它**打断**用户 | **不受 push 护栏限**(不打断 = 可全摆出来,comprehensive) |
| 形态 | 一句话 peek + emote | 卡(墙模式看全 / Tinder 左滑跳过·右滑执行) |

- **共享状态**:peek 里 dismiss → Offer 屏同步标记;Offer 屏 execute → peek 不再弹同一条。一条 offer = 一个实体,两个面。
- **纯通知不进 Offer 屏**:闪念完成/任务做完/到点提醒 = 已发生的播报 → 只进**通知 feed**(§14.7),**不进** Offer 屏(Offer 屏只放「你还能做的事」)。

**dismiss(左滑跳过)= 软「今天不想做」(2026-06 定):**
- 跳过 → 移出**今天**的活跃 Offer deck + 落**通知 feed**(留 14 天,可翻回做,§14.7)。
- **压一天**:当天**再进 app 不再 offer 它**;**第二天**条件若仍成立 → **重新 offer**。(屏内 **↻ 重新生成** = 当次把刚跳过的重发一遍,当场反悔。)
- 落地:dismiss 标 `nudges` outcome=`dismissed`(已有时间戳);**PULL 现算时过滤掉「dismiss 的北京日 == 今天」的 offer**,跨日自然解除。**§14.8 的 72h/7d 退避只挡 PUSH,不挡 PULL**(用户主动来翻,理应看到全集)。

**execute(右滑执行)** = 等同 peek 的「一键即做」(标 acted → 跑 §6 报告管线;习惯类 → 开该技能快记)。

**watermark(增量,承 §14.3 不变)**:执行后水位线前移,**只有新攒满阈值才再生成 offer** —— 总结过的旧批不重复(10 篇灵感总结过 → 再攒 10 篇**新的**才再 offer,且**只总结新批**;记账消费总结、单词 quiz 同理走「上次总结后的新增」)。
...
> **并入今日页（新，系 [§4.5.0](04-frontend.md)）**：晨报不再是独立的一次性页，而是**今日页（首页 tab0）的「上午首开 hero」**—— 北京时间中午前当天首开 → 今日页顶部展开晨报 hero（早安 + 今日预览 + 昨日回顾 + 本周进度），**下滑即今天实时内容**（§4.5.0a 时段流 + 闪念 chip）；✕ /「开始今天」收起 hero、当天不再展开。**三级 gating（下条）不变**，只是承载从「独立沉浸页」改为「今日页 hero 段」；`morning_briefing_page.dart` 的沉浸皮迁为今日页 hero 皮。
...
- **图标 = Kenney emote(CC0,见 [§9.2 emote 叠层](09-pet.md))**:peek 气泡 + feed 行的前导图标按类型映射 —— 整理灵感 `idea`💡 · 记账 `cash`💰 · 节律缺口 `dots3`(或按域:健康 `heart`、记账 `cash`)· 到点提醒 `exclamation`❗ · 完成/acted `faceHappy`/`stars`。**gentle-only:不用 `faceAngry`/`faceSad`/`heartBroken`/`anger`**(承 §14.8 不愧疚)。
...
- **持久化(2026-06 补明确,修"重进就没了")**:① feed **开 app 从 `GET /api/notifications` 拉历史**(不只 `/api/nudges/pending` 恢复 pending),重进不丢;② **dismiss(知道了)≠ 删除** —— 只置 `read` / `status=dismissed`,通知行**留 feed**(灰),「整理灵感 / 该记账」明天还能翻出来做;③ 通知(含 nudge)**保留 14 天**再 prune([§2 §3.14](02-data-model.md))。feed = 一段「Reka 建议过 + 完成过的事」的近 14 天历史。
