...
| `quiz` | **学习类知识内容** + 想被**考/打分**(§6.14) | 「考考我这周记的单词」「出一份测验」 |
| `flashcard` | **学习类知识内容** + 想**自测/翻牌/背**(§6.14) | 「把这周的新词做成记忆卡」「帮我背一背」 |
...
字段说明:
- `genre`(必填):七选一(`data-report` / `idea-synthesis` / `proposal` / `digest` / `briefing` / `quiz` / `flashcard`)。
