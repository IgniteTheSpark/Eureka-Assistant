...
def make_dispatcher_agent(custom_skills_hint: str = "") -> LlmAgent:
    ...
    if custom_skills_hint:
        prompt += (
            ...
            "**这个『优先』只压过 misc / notes / idea,绝不压过 todo / event / "
            "contact / expense。** 未来要做/要去的安排(有时间点 + 有一场/参加/去/开会/打球/"
            "比赛/球赛/训练/课程/约/聚)按上面的规则归 **todo 或 event**,**不是**运动/skill 记录 —— 运动类 "
            "skill 记的是**已发生的锻炼数据**(「跑了 5 公里 配速 6」),不是「下午 4 点半"
            "有球赛参加 / 今天 11 点有一场网球比赛」这种日程安排。**别因为活动落在『运动』domain(打球/球赛/比赛)就硬塞进"
            "跑步 skill**;domain 只是给条目打的标签,跟选哪个 skill 是两码事。\n\n"
            "示例输出:\n"
            "```json\n"
...
def make_custom_skill_agent(
    ...
    instruction = (
        ...
        f"3. asset 级落段参数也必须抽取并传给 tool_create_asset(所有自定义 skill 都一样):\n"
        f"   - 说了具体钟点(「下午3点」「早上8点」「刚刚/现在/几分钟前」)→ "
        f"occurred_at=\"YYYY-MM-DDTHH:mm:ss+08:00\";可同时传 period。\n"
        f"   - 只说模糊时段(「早上/上午/中午/下午/晚上」)→ "
        f"period=\"上午/中午/下午/晚上\"，occurred_at=\"\";**严禁**把「下午」写成 15:00。\n"
        f"   - 说了相对日期但没钟点(「昨天/前天/明天」)→ created_at=\"那天T00:00:00+08:00\" "
        f"用来把记录归到那天;如果同时说「昨天下午」还要 period=\"下午\"。\n"
        f"   - 如果 schema/render_spec 有 date / played_at / occurred_at / timeline_anchor 这类发生日期字段,"
        f"也在 payload 里按同一日期填写;但不要制造假钟点。\n"
        f"   - 啥时间都没说 → period=\"\", occurred_at=\"\", created_at=\"\"，让捕捉时刻兜底。\n"
        f"4. 调用 tool_create_asset:\n"
        f"     user_skill_name=\"{skill_name}\"\n"
        f"     payload=JSON 字符串\n"
        f"     session_id=<上面给的>\n"
        f"     source_input_turn_id=<上面给的>\n"
        f"     period=<按上面规则,没有则空字符串>\n"
        f"     occurred_at=<按上面规则,没有则空字符串>\n"
        f"     created_at=<只在用户明说日期且没具体钟点时传,否则空字符串>\n"
        f"5. 工具返回后,**只输出**(不要别的话):\n"
        f"```json\n"
