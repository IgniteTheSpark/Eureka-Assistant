...
**每次** create **都必须**按内容判定一个 domain(8 选 1:工作 / 学习 / 健康 / 运动 / 社交 / 娱乐 / 生活 / 灵感),
作为 create 工具的 `domain` 参传进去,**永远不留空**:
...
- **每条都打,绝不留空**——尤其**自定义技能没有默认域**(如喝水/跑步技能),你不打它就是 null、卡片不显领域。
  「喝了100ml水」就该 `domain=健康`,「下班买菜」就该 `domain=生活`。
- **实在判不准 / 泛泛的日常杂事 → 落「生活」兜底**,不要留空。这是个**轻标签**,别纠结边界、**更别因此追问用户**。
